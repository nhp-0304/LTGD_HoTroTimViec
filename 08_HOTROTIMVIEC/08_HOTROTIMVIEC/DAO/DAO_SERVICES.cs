...
namespace _08_HOTROTIMVIEC.DAO
{
    class DAO_SERVICES
    {
        ...
        public void Dky_NLD_Image(int maNLD, string link, int kind)
        {
            conn.SV_Dky_NLD_Image(maNLD, link, kind);
        }

        public void Add_ThongTin_TaiKhoan_DVTD(string maDV, string tenDV, string diachi, string sdt, string mk)
        {
            conn.SV_Add_ThongTin_TaiKhoan_DVTD(maDV,tenDV,diachi,sdt,mk);
        }
        public void Dky_DVTD_Image(string maDV,string link, int kind)
        {
            conn.SV_Dky_DVTD_Image(maDV,link,kind);
        }
        public int? Check_DVTD_image_TrungTen(string tenImage)
        {
            int? check = 0;
            conn.SV_Check_DVTD_image_TrungTen(tenImage, ref check);
            return check;
        }
        public string getLinkNLD(int maNLD)
        {
            var link = (from s in conn.NGUOILD_Images where s.MaNLD == maNLD select s).FirstOrDefault();
            return link.Link;
        }
        
        public string getLinkDV(string maDV)
        {
            return (from s in conn.DONVI_Images where s.MaDV == maDV select s).FirstOrDefault().Link;
        }

        public void UpdateImage_Link_DV(string maDV, string link)
        {
            conn.SP_UpdateImage_Link_DV(maDV, link);
        }
    }
}
